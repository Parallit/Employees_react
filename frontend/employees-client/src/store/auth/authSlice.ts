...
export const userRegistration = createAsyncThunk(
    ...
    async (payload: { name: string, email: string, password: string }, thunkApi) => {
        try {
            ...
            return thunkApi.rejectWithValue(`Не удалось зарегистрировать пользователя: ${error}`)
        }
    })
...
export const checkAuthUser = createAsyncThunk(
    ...
    async (_, thunkApi) => {
        try {
            ...
            return thunkApi.rejectWithValue(`Не удается получить информацию об авторизации пользователя: ${error}`)
        }
    }
)
